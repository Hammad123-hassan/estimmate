...
export default function EstimateInput({ onBuild }) {
  ...
  return (
    <div className="mx-auto w-full max-w-3xl">
      <div className="rounded-2xl border border-brand-200/80 bg-white p-3 shadow-[0_20px_60px_-20px_rgba(91,110,242,0.35)] sm:rounded-[28px] sm:p-5">
        <textarea
          value={brief}
          onChange={(e) => setBrief(e.target.value)}
          rows={3}
          className="estimate-textarea w-full resize-none border-0 bg-transparent text-sm leading-relaxed text-ink outline-none sm:text-base"
          placeholder="Describe your project…"
          aria-label="Project brief"
        />

        <div className="mt-3 flex flex-col gap-3 border-t border-brand-50 pt-3 sm:flex-row sm:flex-wrap sm:items-center sm:gap-2">
          <div className="flex min-w-0 flex-1 flex-wrap items-center gap-2">
            <button
              type="button"
              className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-brand-100 text-brand-600 transition hover:bg-brand-200"
          ...
          <Button
            ...
            className="w-full shrink-0 sm:ml-auto sm:w-auto sm:min-w-30"
          >
            {building ? (
              <>
      ...
      {result && (
        <div className="mt-4 animate-[fadeUp_0.4s_ease] rounded-2xl border border-brand-200 bg-white/90 p-3 shadow-lg shadow-brand-500/10 backdrop-blur sm:p-4">
          <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-start sm:justify-between sm:gap-3">
            <div className="min-w-0">
              <p className="text-xs font-semibold uppercase tracking-wide text-brand-500">Estimate ready</p>
              <p className="mt-1 wrap-break-word font-semibold text-ink">{result.title}</p>
          ...
          <div ref={metricsRef} className="mt-4 grid grid-cols-1 gap-2 min-[400px]:grid-cols-3 sm:gap-3">
...
function Metric({ label, value, tone }) {
  return (
    <div data-stagger className="rounded-xl bg-brand-50/80 px-2 py-2.5 text-center min-[400px]:px-3">
      <p className="text-[10px] font-medium uppercase tracking-wide text-ink-muted sm:text-[11px]">{label}</p>
      <p className={`mt-0.5 text-sm font-bold tabular-nums sm:text-base ${tone}`}>{value}</p>
    </div>
  )
}
  ...
  return (
    <div className="relative">
      <button
        type="button"
        onClick={onToggle}
        className="inline-flex max-w-full items-center gap-1.5 rounded-full border border-brand-200 bg-brand-50/60 px-2.5 py-1.5 text-xs font-medium text-ink transition hover:bg-brand-100 sm:px-3 sm:text-sm"
      >
        {accent && (
        ...
        <ChevronDown className={`h-3.5 w-3.5 shrink-0 text-ink-muted transition ${open ? 'rotate-180' : ''}`} />
      </button>
      {open && (
